...
class BaseDataConfig(ABC):
    def __post_init__(self):
        """Set default indices if not specified."""
        # For backward compatibility, use observation_indices if separate indices not set
        if hasattr(self, "observation_indices"):
            if (
                not hasattr(self, "video_observation_indices")
                or self.video_observation_indices is None
            ):
                self.video_observation_indices = self.observation_indices
            if (
                not hasattr(self, "state_observation_indices")
                or self.state_observation_indices is None
            ):
                self.state_observation_indices = self.observation_indices

    def modality_config(self) -> dict[str, ModalityConfig]:
        # Initialize separate indices from observation_indices if not already set
        if hasattr(self, "observation_indices") and not hasattr(self, "video_observation_indices"):
            self.video_observation_indices = self.observation_indices
            self.state_observation_indices = self.observation_indices
        ...
        language_modality = ModalityConfig(
            delta_indices=self.video_observation_indices,  # Use video indices for language
...
class UCRWBLMMobyHistoryDataConfig(BaseDataConfig):
    """
    UCR data config with whole-body motion (WBM) actions.
    ...
    state_observation_indices = [-15, -12, -9, -6, -3, 0]
