#!/usr/bin/env python3
"""
UCR GR00T Client - UCR Branch Legacy Style

This mimics the old ucr branch client approach:
- Uses HTTP POST to /act endpoint
- Uses FLAT observation format (video.ego_view, state.waist_joint, etc.)
- Compatible with old UCR branch code

Equivalent to old ucr branch usage.
...
def create_dummy_observation_flat(batch_size=1):
    """
    Create a dummy observation with FLAT format (UCR branch style).

    This uses the flat key format like 'video.ego_view', 'state.waist_joint'
    instead of nested dicts like {'video': {'ego_view': ...}}

    Returns:
        Dictionary with flat keys matching ucr branch format
    """
    ...
    observation = {
        ...
        # Language - tuple/list format (UCR branch style)
        "task": ["pick up the box"],  # Simple list, not nested
    }
...
def send_observation_http(observation):
    """
...
def main():
    ...
    # Create dummy observation in FLAT format
    print("Creating dummy observation (FLAT format)...")
    obs = create_dummy_observation_flat(batch_size=1)
    ...
    try:
        actions = send_observation_http(obs)
        ...
        # Show example action values
        print("Example action values (first timestep):")
        for action_key, action_value in sorted(actions.items()):
            if "position" in action_key:
                print(f"  {action_key}[0,0,:]: {action_value[0,0,:]}")
